...
<section class="p-0">
    <div class="slide-1 home-slider">
        <?php foreach ($banners as $key => $value)  {?>
            <div>
                <div class="home text-center">
                    <img src="<?= base_url()?>images/banner_images/<?= $value['banner_image']?>" alt="" class="bg-img blur-up lazyload">
                    <div class="container">
                        <div class="row">
                            <div class="col">
                                <div class="slider-contain">
                                    <div>
                                        <!-- <h4>save 10%</h4> -->
                                        <h1><?=$value['banner_text']?></h1><a href="<?= $value['banner_link'] ?>" class="btn btn-solid">shop now</a>
                                    </div>
                                </div>
                            </div>
                        </div>
       ...
       <!--  <div>
            <div class="home text-center">
                <img src="<?=base_url()?>/assets/images/home-banner/banner-1.jpg" alt="" class="bg-img blur-up lazyload">
                <div class="container">
                    <div class="row">
                        <div class="col">
                            <div class="slider-contain">
        ...
        <div class="row">
            <div class="col">
                ...
                <div class="product-5 product-m no-arrow">
                    <?php foreach ($trending_products as $key => $value) {?>
                        <div class="product-box product-wrap">
                            <div class="img-wrapper">
                                <!-- <div class="lable-block"><span class="lable3">new</span> <span class="lable4">on sale12</span></div> -->
                                <div class="front">
                                    <a href="<?= base_url('productdetail/'.$value['link'].'')?>">
                                        <?php if (!empty($value['product_thumbnail_image'])) {?>
                                            <img src="<?= base_url('images/products_images/'.$value['product_thumbnail_image']) ?>" class="img-fluid bg-img blur-up lazyload" alt="">
                                       <?php }else{?>
                                            <img src="<?= base_url('assets/images/logo.png') ?>" class="img-fluid bg-img blur-up lazyload" alt="">
                                        <?php } ?>
                                ...
                                <div class="cart-info cart-wrap">
                                    ...
                                    <a href="javascript:void(0)" onclick="getQuickview('<?=$value['product_id']?>')"><i class="ti-search" aria-hidden="true"></i></a>
                                </div>
                                <div class="addtocart_btn">
                                    <!-- <button class="add-button add_cart" title="Add to cart" onclick="addToCart(<?=$value['product_id']?>,<?=$value['default_price']?>,<?=$value['category_id']?>,<?=$value['default_weight']?>)">
                                        add to cart
                                    </button> -->
                                    <a href="<?= base_url('productdetail/'.$value['link'].'')?>"><button  class="add-button" title="View Product">View Product</button></a>
                                    <div class="qty-box cart_qty">
                                        <div class="input-group">
                                            <button type="button" class="btn quantity-left-minus" data-type="minus" data-field="" onclick="minusfromCart(<?=$value['product_id']?>,<?=$value['default_price']?>,<?=$value['category_id']?>,<?=$value['default_weight']?>)">
                                                <i class="fa fa-minus" aria-hidden="true"></i>
                                            </button>
                                            <input type="text" name="quantity" class="form-control input-number qty-input" value="1">
                                            <button type="button" class="btn quantity-right-plus" data-type="plus" data-field="" onclick="addToCart(<?=$value['product_id']?>,<?=$value['default_price']?>,<?=$value['category_id']?>,<?=$value['default_weight']?>)">
                        ...
                        <div class="product-detail text-center">
                            ...
                            <h4>							&#8377; <?= $value['default_price']?> 														<small>Onwards</small>							</h4>							<?php  if($value['category_id'] == 6 ){?>                                 <span style="width: 89px;float:right">per/kg </span>                            <?php } ?>
...
<section class="p-0">
    <div class="full-banner parallax text-left p-left">
        <img src="<?= base_url()?>assets/images/parallax/home.jpg" alt="" class="bg-img blur-up lazyload">
        <div class="container">
            <div class="row">
                <div class="col">
                                ...
                                <?php  foreach ($new_products as $key => $value) {?>
                                    <div class="product-box">
                                        <div class="img-wrapper">
                                            ...
                                            <div class="front">
                                                <!-- <a href="product-page(no-sidebar).html"><img src="<?= base_url()?>assets/images/products/4.jpg" class="img-fluid bg-img blur-up lazyload" alt=""></a> -->
                                                <a href="<?= base_url('productdetail/'.$value['link'].'')?>">
                                                    <!-- <img src="<?= base_url('images/products_images/'.$value['product_thumbnail_image']) ?>" class="img-fluid bg-img blur-up lazyload" alt="">
 -->                                            <?php if (!empty($value['product_thumbnail_image'])) {?>
                                                    <img src="<?= base_url('images/products_images/'.$value['product_thumbnail_image']) ?>" class="img-fluid bg-img blur-up lazyload" alt="">
                                           ...
                                           <!--  <div class="add-button" data-toggle="modal" data-target="#addtocart" onclick="addToCart(<?=$value['product_id']?>,<?=$value['default_price']?>,<?=$value['category_id']?>,<?=$value['default_weight']?>)">add to cart</div> -->
                                           <a href="<?= base_url('productdetail/'.$value['link'].'')?>"><button  class="add-button" title="View Product">View Product</button></a>
                                        </div>
                                        <div class="product-detail">
                                            
                                            <a href="<?= base_url('productdetail/'.$value['link'].'')?>"><h6><?= $value['product_name'] ?></h6></a>
                                            <h4>											&#8377; <?= $value['default_price']?>																						<small>Onwards</small>											</h4>											<?php  if($value['category_id'] == 6 ){?>                                                     <span style="width: 89px;float:right">per/kg </span>                                                <?php } ?>
                            ...
                            <?php foreach ($best_products as $key => $value) {?>
                                <div class="product-box">
                                    ...
                                    <div class="product-detail">
                                        ...
                                        <a href="<?= base_url('productdetail/'.$value['link'].'')?>"><h6><?= $value['product_name']?></h6></a>
                                        <h4>										&#8377; <?= $value['default_price']?> per/kg																				<small>Onwards</small> </h4>										<?php  if($value['category_id'] == 6 ){?>                                                  <span style="width: 89px;float:right">per/kg </span>                                            <?php } ?>
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-lg-6 col-xs-12">
                        <div class="quick-view-img"><img src="<?= base_url()?>assets/images/pro/1.jpg" alt="" class="img-fluid blur-up lazyload"></div>
                    </div>
...
<script>
    ...
    function getQuickview(product_id){
	if(product_id)
	{
		$.ajax({
			url:"<?php echo base_url();?>productlisting/getQuickview",
			data:{product_id:product_id},
			type:'post',
			dataType: 'json',
			success: function(res)
			{
				$("#Quickviewmodel").html(res['quickview']);
...
function getweight(product_size){
    var product_id = $("#product_id").val();
    if(product_size !="" && product_id !="")
	{
		$.ajax({
			url:"<?php echo base_url();?>productdetail/getProductVarient",
			data:{product_id,product_size},
			type:'post',
			dataType: 'json',
			success: function(res)
			{
				...
				$("#product-varient").html(res['varient']);
                // $("#product-price-sm").html(res['varient-price']);
                // $("#product-price").html(res['varient-price']);
        ...
        $("#default-weight").show();
        $("#product-varient").hide();
        $("#product-price").html($("#product_price").val());
    }
}

function setweight(productVarient_id){
    if(productVarient_id){

        console.log('#weight-'+productVarient_id);
        // $('#weight-'+weight_id).addClass("Active");
        // $(this).parent().addClass('active');
        
            $.ajax({
                url:"<?php echo base_url();?>productdetail/getProductVarientPrice",
                data:{productVarient_id,productVarient_id},
                type:'post',
                dataType: 'json',
                success: function(res)
                {
...
function addToCart(item_id,item_price,category_id,default_weight){
    $.ajax({
    url:"<?php echo base_url()?>home/addToCart",
    dataType:"json",
    data:{"item_id":item_id,"item_price":item_price,"category_id":category_id,"default_weight":default_weight},
    type:"POST",
    success:function(response){
        ...
        $('#cartview').html('');
        $('#cartview').html(response.data);
        $('#cartview').css('display','block');
        
      } 
    });
}
function minusfromCart(item_id,item_price,category_id,default_weight){
